/**
 * Kit Feirinha / Preço médio de revenda — cálculos compartilhados (API + validação).
 * Mantém paridade com public/assets/feirinha-calc.js
 */
...
/** Itens ativos do pedido → totais para feirinha (usa vlrtotal_itens e quantidade). */
...
/**
 * @param {Array} itens — linhas do pedido (_subData.itens)
 * @param {{ precoRevenda?: number|null }} opts
 */
...
/** Teto de preço unitário de custo para compor a faixa (ex.: R10 → 7,99). */
...
/**
 * Quantidade adicional de um produto barato para atingir meta de preço médio.
 * @returns {{ qtd: number, alreadyOk: boolean }|null}
 */
function calcQtdParaAtingirMedia(valorTotal, qtdTotal, precoUnitNovo, metaMedia) {
  ...
  if (Q <= 0) return null;
  const currentAvg = V / Q;
  if (currentAvg <= meta + 0.0001) return { qtd: 0, alreadyOk: true };
...
function calcFeirinhaResumo(itens, opts = {}) {
  ...
  if (opts.faixaCodigo) {
    faixa = { ...getFaixaByCodigo(opts.faixaCodigo), precoMedio: agg.precoMedio };
  }
