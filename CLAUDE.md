...
- `segmentos.uso` = **`CLIENTE`** | **`FORNECEDOR`** | **`AMBOS`** (default) — combo cliente usa `?uso=CLIENTE`, fornecedor/transportadora `?uso=FORNECEDOR` (sempre inclui AMBOS); produto NÃO usa `segmentos` — usa tabela **`categoria`** (`/api/lookups/categorias` / menu Categoria de Produto)
- **Tipo de Cliente**: combo NÃO é mais hardcoded — tabela **`tipo_cliente`** (`codigo`, `descricao`, `ordem`, `status`); `clientes.tipo_cliente` grava o **codigo** (ex. `CONSUMIDOR`); lookup `GET /api/lookups/tipo-cliente`; CRUD `/api/tipo-cliente` + tela `/pages/tipo-cliente.html`; seed padrão Consumidor/Revendedor/Especial/Indústria em `config/tipo-cliente-migrate.js`
- **Auxiliares do produto** (Grupo / Subfamília / Unidade / Tipo de Grade / Local): tela **`/pages/produto-auxiliares.html`** + API `/api/produto-auxiliares/*`; schema/seed em `config/produto-auxiliares-schema.js`. Local do produto = tabela **`local_armazenamento`** (`nome_local`) — a tela antiga `locais-armazenamento` também grava nela (não usar `locais_armazenamento` órfã). Lookups: `/api/lookups/grupos`, `subfamilias`, `unidades`, `tipos-grade-produto`, `local-arm`. Importação auto-cria via `config/produto-lookup-import.js`
- **Confirm/exclusão no iframe da home**: NÃO usar `position:fixed` no overlay/card — em várias telas o fixed cai no fluxo do documento e o diálogo só aparece após rolar até o fim da lista. Usar **`/assets/confirm-dialog.js`** (`confirmar(titulo, msg, onOk)` ou `SysRepConfirm.ask({...})`) — overlay `absolute` na área visível (`top=scrollY`, `height=innerHeight`) + flex center. Regra Cursor: `.cursor/rules/confirm-dialog-iframe.mdc`
- lookups de `regiao_rota` e `categoria` retornam **`descricao`**, NÃO `nome`
- **Segmento de cliente ≠ categoria de produto**: combo e validação usam tabela **`segmentos`** (`/api/categorias` + `resolveSegmentoCliente`); `categoria` é só produto — validar em `categoria` ao salvar cliente gerava «Categoria (segmento) informada não existe ou está inativa» (bug 2026-07-25)
- Cliente ↔ fábricas: tabela **`cliente_representadas`** (`cod_cliente`, `cod_fornecedor`, `codigo_na_fabrica`, **`marca_propria`**, **`nome_marca`**, **`logo_caminho`**) + flag **`clientes.restringe_representadas`** (default `'N'`); com `'S'`, `GET /api/fornecedores?somente_fabricas=true&cod_cliente=` filtra só as vinculadas; logo via `POST /api/clientes/:id/representadas/logo`
...
- `receber.status` no Delphi legado é **`A RECEBER`** (não `ABERTA`) — filtros/lote/KPIs devem tratar aberto = *não liquidado* via `config/receber-status.js` (`sqlAberto`/`sqlLiquidado`); baixa grava **`RECEBIDO`**; `forma_pagto` pode ser **VARCHAR(15)** → truncar com `truncFormaPagto`
- `regioes` (tabela) → **não usar**; regiões reais estão em **`regiao_rota`** — `/api/regioes` em `routes/cadastros.js` deve ler/gravar `regiao_rota` (a tela Gestão de Regiões apontava para tabela órfã vazia)
- `motivo_visitas` / `visitas` no Delphi: soft-delete costuma ser **`exluido`** (typo, sem C) — a web nova usa **`excluido`**; CRUD de Motivo de Visitas detecta a coluna real (`resolveMotivoVisitasSoftCol`) — gravar só `excluido` em base legada gerava «Unknown column 'excluido'» (bug 2026-07-27)
...
- `pedidos` NÃO tem coluna `cod_vendedor` (essa é de **`clientes`**) — vendedor do pedido é **`id_usuario`** (FK `usuarios.idusuario`); JOIN padrão `LEFT JOIN usuarios u ON u.idusuario = p.id_usuario` (também existe snapshot `nome_vendedor`); bug 2026-07-25: diário do cliente usava `p.cod_vendedor` e falhava com Unknown column
...
- Plano de contas **gerencial** (não partida dobrada): tabela **`plano_contas`** (hierarquia `id_pai`, `tipo` SINTETICA/ANALITICA, `grupo`, `aceita_lancamento`) + **`centro_custo`**; vínculo padrão em **`despesas.id_planoconta`** e **`natureza.id_planoconta`**; override opcional em **`pagar`/`receber`**: `id_planoconta`, `id_centrocusto`; resolução = `COALESCE(titulo.id_planoconta, despesa|natureza.id_planoconta)`; schema em `config/plano-contas-schema.js`; CRUD `/api/plano-contas` e `/api/centro-custo`; balancete `GET /api/contabil/balancete`; telas `plano-contas.html`, `centro-custo.html`, `balancete.html`
- `plano_contas.numero_pai` → coluna **legado Delphi** (NOT NULL sem default em várias bases); a web usa `id_pai` — em INSERT/UPDATE/seed preencher `numero_pai` com o `numero` do pai (ou `''`) via `planoContasLegacyWriteFields` / `ensureNumeroPaiCompat` em `config/plano-contas-schema.js` (bug 2026-07-16: «Field numero_pai doesn't have a default value»)
- Referência mãe / cores (Mercos-style, opt-in): coluna **`produto.id_referencia`** (NULL = avulso ou mãe) aponta para o produto mãe; cada cor continua SKU pedível; helper `config/produto-referencia.js` (+ `resolverReferenciaMaeImport`); API `GET /api/produtos/referencias-mae`, `GET /api/produtos/:id/variacoes`; cadastro aba Outras com **+ Adicionar cor** (clona + vincula); importação campo virtual **`referencia_mae`** → `id_referencia` (mães antes das cores); Catálogos Visuais checkbox «Incluir cores ao adicionar referência mãe»; Showroom (`catalogo-afv-ui.js`) `agrupar_referencia=1` + chips de cor — carrinho no SKU da cor
- DRE (`routes/dre.js`) e BI financeiro (`routes/financeiro.js`): despesas de Pagar classificam por **`id_despesas` → despesas** (NÃO por `id_natureza`/`natureza`); gráficos/DRE também agregam por plano de contas resolvido — nunca juntar `pagar` com `natureza` para categoria de despesa
...
- **Importação de produtos/preços — "Valor de Venda não importa" (CAUSA REAL):** `autoMapear()` em `public/pages/importacao-precos.html` só auto-mapeava 3 campos (`AUTO_MAP_APENAS` = descricao/unidade/cod_fabricante) → **`Valor de Venda` (e todos os demais campos da planilha-modelo) ficavam SEM vínculo**; se o usuário não mapeasse na mão, o produto importava ("importado") **sem preço**. Não tinha relação com inteiro/float. Fix: `autoMapear()` faz **match exato** do cabeçalho com `nome_campo` OU `apelido` de qualquer campo ativo (os cabeçalhos do modelo são exatamente os apelidos). **Exceção `AUTO_MAP_BLOQUEADOS`**: `estoque_atual`/`estoque_minimo`/`estoque_maximo`/`estoque_seguranca`/`situacao` **nunca** auto-mapeiam (têm efeito colateral — estoque gera movimento AJUSTE, situacao muda ativo/inativo); exigem mapeamento manual, pra importação de preço/tabela não mexer em estoque/status sem o usuário pedir. Verificar com a planilha-modelo real antes de mexer.
  ...
  - **Aliases de cabeçalho:** `Valor`/`Valor de Venda`/`Preço` → `vlr_venda`; normalização remove acento/ponto (`Cod. Fabricante` = `Cód. Fabricante`). Campo virtual **`grade`** (apelido Grade) → resolve para **`produto.tipograde`** via `config/produto-grade-import.js` (nome/apelido ou tamanhos `P/M/G/GG`); **não cria** grade — se não achar, a linha falha com aviso.
...
- `clientes.complemento` — **não existe** em bases Delphi antigas; migration em `schema-migrations.js` + listagem usa `NULL AS complemento` se a coluna ainda não existir (`modules/clientes/clientes.repository.js`)
- Diário comercial do cliente: tabela **`cliente_diario`** + timeline em `GET /api/clientes/:id/diario` (une diário + visitas + pedidos); aba **Diário** em `clientes.html`; finalizar visita grava `resultado`/`motivo_resultado`/`conversa` em `visitas`
- `clientes` NÃO tem `razaosocial`/`nomefantasia` — nome do cliente é **`nome`** (fallback **`apelido`**); faturamento Delphi usa `razao`/`fantasia` em tabela auxiliar, não no JOIN padrão de listagens
- `POST /api/pedidos/bulk-update`: **não** gravar `pedidos.status` com texto de `situacao_pedido` (`APROVADO` etc.) — em bases Delphi `status` costuma ser **CHAR(1)/VARCHAR(1)** e a transação falhava inteira; atualizar só `situacao_pedido` (+ `tipo_pedido` se vier)
